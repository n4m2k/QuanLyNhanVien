...
const ApproveDialog = ({ open , onClose,  employee , statusOfForm , closeDialogDetail}) => {
  ...
  const handleApproveSubmit = () => {
    switch (statusOfForm) {
      ...
      default:
        if (Number(employee?.submitProfileStatus) === STATUS_EMPLOYEE.CHO_DUYET_KET_THUC.CODE) {
        ...
        } else if (Number(employee?.submitProfileStatus) === STATUS_EMPLOYEE.CHO_DUYET.CODE) {
  ...
  return (
    <div>
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        <DialogTitle> <span className="styleColor">Phê duyệt</span></DialogTitle>
        ...
        <DialogContent>
          <TextValidator
            ...
            value={
              dataTemp
                ? moment(dataTemp).format("YYYY-MM-DD")
                : dateNow
            }
